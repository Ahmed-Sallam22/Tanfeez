...
export default function TransferDetails() {
    ...
    if (apiData?.transfers && apiData.transfers.length > 0) {
      const initialRows = apiData.transfers.map((transfer) => {
        ...
        const row: TransferTableRow = {
          ...
          // Use primaryBudget data based on transfer_type, otherwise fall back to transfer data
    ...
    try {
      ...
      if (response?.transfers) {
        if (hasErrors) {
          ...
          toast.error(t("validation.hasErrors"));
        } else if (response.summary?.balanced) {
          toast.success(t("transfer.saveSuccess"));
        } else {
          toast.success(t("transfer.saveSuccess"));
        }
      } else {
  ...
  const handleSubmit = async () => {
    if (!isSubmitDisabled()) {
      ...
      try {
        ...
        // Show success toast
        toast.success(t("transfer.submitSuccess"));
  ...
  const fetchFinancialDataForRow = async (
    ...
    try {
      ...
      console.log(
        `Row ${rowId}: Transfer type is "${apiData?.summary?.transfer_type}", using ${recordToUse?.Control_budget_name} for main values`
      );
      ...
      // Apply financial data using the selected record based on transfer_type
  ...
  const columns: TableColumn[] = [
    ...
    {
      id: "accountId",
      header: t("tableColumns.accountCode"),
      render: (_, row) => {
        const detailRow = row as unknown as TransferDetailRow;
        return (
    ...
    {
      id: "accountName",
      header: t("tableColumns.accountName"),
      render: (_, row) => {
        const detailRow = row as unknown as TransferDetailRow;
        return (
    ...
    {
      id: "approvedBudget",
      header: t("tableColumns.approvedBudget"),
      showSum: true,
      render: (_, row) => {
        const detailRow = row as unknown as TransferDetailRow;
        return (
    ...
    {
      id: "availableBudget",
      header: t("tableColumns.availableBudget"),
      showSum: true,
      render: (_, row) => {
        const detailRow = row as unknown as TransferDetailRow;
        return (
  ...
  const columnsDetails: TableColumn[] = [
    ...
    {
      id: "encumbrance",
      header: t("tableColumns.encumbrance"),
      showSum: true,
    ...
    {
      id: "commitments",
      header: t("tableColumns.commitments"),
      showSum: true,
    ...
    {
      id: "obligations",
      header: t("tableColumns.obligations"),
      showSum: true,
    ...
    },
    {
      ...
      header: t("tableColumns.otherConsumption"),
      showSum: true,
    ...
    {
      id: "total_budget",
      header: t("tableColumns.totalBudget"),
      showSum: true,
    ...
    {
      id: "initial_budget",
      header: t("tableColumns.initialBudget"),
      showSum: true,
    ...
    },
    {
      ...
      header: t("tableColumns.budgetAdjustments"),
      showSum: true,
    ...
    {
      id: "other_ytd",
      header: t("tableColumns.otherYtd"),
      showSum: true,
    ...
    {
      id: "costValue",
      header: t("tableColumns.costValue"),
      showSum: true,
  ...
  const handleReopenRequest = async () => {
    try {
      ...
      toast.success(t("transfer.reopenSuccess"));
    ...
    <div>
      ...
      <div>
        ...
        <SharedTable
          ...
          addRowButtonText={t("common.addRow")}
        ...
        {!isSubmitted && isSaving && (
          <div className="flex justify-end mt-4 p-4 bg-white rounded-lg shadow-sm">
            <div className="flex items-center gap-2 text-blue-600">
              ...
              <span className="text-sm">{t("messages.savingData")}</span>
            </div>
          </div>
        )}
      </div>
            ...
            <div className="mt-3 flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
              ...
              <span>
                {(() => {
                  ...
                  if (totalValidRows < 2) {
                    return t("validation.minRows");
                  } else {
                    return t("validation.hasErrors");
                  }
                })()}
              </span>
            </div>
      ...
      >
        <div className="p-4 ">
          <div className="bg-[#F6F6F6] rounded-lg p-3">
            <h2 className="text-md  font-medium mb-4">{t("common.summary")}</h2>
